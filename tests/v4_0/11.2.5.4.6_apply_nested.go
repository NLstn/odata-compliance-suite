...
func ApplyNested() *framework.TestSuite {
    suite := framework.NewTestSuite("11.2.5.4.6 $apply nested transformations", "Tests nest output structure and nested transformation semantics.", "https://docs.oasis-open.org/odata/odata-data-aggregation-ext/v4.0/odata-data-aggregation-ext-v4.0.html#sec_Transformationnest")
    suite.AddTest("test_apply_nest_groupby", "nest returns one row containing the transformed input collection", func(ctx *framework.TestContext) error {
        ...
        if !ok || len(categories) == 0 { return fmt.Errorf("Categories must be a non-empty array, got %#v", body.Value[0]["Categories"]) }
        return nil
    })
    return suite
}
