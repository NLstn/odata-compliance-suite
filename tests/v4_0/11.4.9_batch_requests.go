...
--batch_boundary--`
	...
	suite.AddTest(
		...
		"A multipart part inherits outer OData-Version and OData-MaxVersion",
		func(ctx *framework.TestContext) error {
...
--batch_version_inherit--`, payload)
			...
			if !strings.Contains(body, "http/1.1 2") {
				return fmt.Errorf("inherited-version part did not succeed: %s", string(resp.Body))
			}
			if !strings.Contains(body, "odata-version: 4.0") {
				return fmt.Errorf("inherited OData-MaxVersion did not produce a 4.0 part response: %s", string(resp.Body))
			}
			return nil
		},
	)
...
--batch_version_override--`, payload)
			...
			body := strings.ToLower(string(resp.Body))
			if !strings.Contains(body, "http/1.1 2") {
				return fmt.Errorf("part with explicit versions did not succeed: %s", string(resp.Body))
			}
			if !strings.Contains(body, "odata-version: 4.0") {
				return fmt.Errorf("part OData-MaxVersion override did not produce a 4.0 response: %s", string(resp.Body))
			}
			return nil
		},
	)
...
--batch_invalid_version--`, payload)
			...
			if !strings.Contains(body, "HTTP/1.1 4") {
				return fmt.Errorf("invalid part payload version did not return inner 4xx: %s", body)
			}
...
--batch_version_rollback--`, firstPayload, secondPayload)
			...
			body := string(resp.Body)
			if !strings.Contains(body, "HTTP/1.1 4") {
				return fmt.Errorf("changeset invalid payload version did not return inner 4xx: %s", body)
			}
