...
func JSONBatch() *framework.TestSuite {
	...
	suite.AddTest(
		...
		func(ctx *framework.TestContext) error {
			...
			r1 := responses["r1"]
			status, ok := r1["status"].(float64)
			if !ok {
				return framework.NewError("inherited-version response is missing a numeric status")
			}
	...
	suite.AddTest(
		...
		func(ctx *framework.TestContext) error {
			...
			status, ok := r1["status"].(float64)
			if !ok {
				return framework.NewError("overridden-version response is missing a numeric status")
			}
	...
	suite.AddTest(
		...
		func(ctx *framework.TestContext) error {
			...
			status, ok := r1["status"].(float64)
			if !ok {
				return framework.NewError("independent-version response is missing a numeric status")
			}
			if status < 200 || status >= 300 {
	...
	suite.AddTest(
		...
		func(ctx *framework.TestContext) error {
			...
			status, ok := responses["r1"]["status"].(float64)
			if !ok {
				return framework.NewError("invalid-version response is missing a numeric status")
			}
			if status < 400 || status >= 500 {
	...
	suite.AddTest(
		...
		func(ctx *framework.TestContext) error {
			...
			for _, id := range []string{"r1", "r2"} {
				status, ok := responses[id]["status"].(float64)
				if !ok {
					return fmt.Errorf("rolled-back request %s is missing a numeric status", id)
				}
				if status < 400 || status >= 500 {
